...
log_section "Eval 28: Stuck Delegated Recovery"
echo ""
echo "╔════════════════════════════════════════════════════════════════════╗"
echo "║           Eval 28: Stuck Delegated Recovery                       ║"
echo "╠════════════════════════════════════════════════════════════════════╣"
echo "║ Test maintenance task recovering steps stuck in DELEGATED state   ║"
echo "║ where the SQS message was lost or never picked up                 ║"
echo "╚════════════════════════════════════════════════════════════════════╝"
echo ""
log_info "Expected Duration: ~30 seconds"
log_info "Expected Outcome: Stuck delegated step re-delegated via SQS"
echo ""
...
###############################################################################
# STEP 3: Simulate stuck DELEGATED state via DB manipulation
###############################################################################

log_section "STEP 3: SIMULATE STUCK DELEGATED STATE"
echo ""

# Find a suitable step (ValidateCustomer) to set back to DELEGATED
...
log_info "  1. Setting ValidateCustomer step back to 'delegated'"
...
log_info "  - SQS message lost or expired"
log_info "  - SQS poller down or misconfigured"
log_info "  - Lambda function deployment broken"
echo ""

# Update step to DELEGATED with old started_at
...
if [ "$STEP_STATUS_CHECK" != "delegated" ]; then
  log_error "DB manipulation failed: step is $STEP_STATUS_CHECK, expected delegated"
  exit 1
fi

###############################################################################
# STEP 4: Trigger stuck-delegated maintenance task
###############################################################################

log_section "STEP 4: TRIGGER MAINTENANCE TASK"
echo ""

log_info "Triggering stuck-delegated maintenance task..."
log_info "API: POST /api/${API_VERSION}/maintenance/tasks/stuck-delegated/execute"
log_info "Options: delegatedTimeoutMinutes=0.25 (15 seconds)"
echo ""
...
STUCK_FOUND=$(echo "$TASK_RESULT" | jq -r '.metrics.stuckDelegatedFound // 0')
RE_DELEGATED=$(echo "$TASK_RESULT" | jq -r '.metrics.reDelegated // 0')
...
log_info "  - Stuck delegated found: $STUCK_FOUND"
log_info "  - Re-delegated: $RE_DELEGATED"
log_info "  - Skipped: $SKIPPED"
log_info "  - Failed: $TASK_FAILED"
echo ""

# Accept either:
# 1. Maintenance task found and re-delegated the step (RE_DELEGATED >= 1)
# 2. Background orchestration auto-fixed it (STUCK_FOUND == 0)
if [ "$STUCK_FOUND" -ge 1 ] && [ "$RE_DELEGATED" -ge 1 ]; then
  log_success "Maintenance task found and re-delegated stuck step"
elif [ "$STUCK_FOUND" -ge 1 ] && [ "$RE_DELEGATED" -eq 0 ]; then
  log_warning "Maintenance task found stuck step but did not re-delegate (skipped=$SKIPPED, failed=$TASK_FAILED)"
  log_info "This could mean the job was auto-fixed by background orchestration"
elif [ "$STUCK_FOUND" -eq 0 ]; then
  log_warning "Maintenance task found no stuck delegated steps - system may have self-healed"
  log_info "Checking current step status..."
fi

###############################################################################
# STEP 5: Wait briefly and verify the step got re-processed
###############################################################################

log_section "STEP 5: VERIFY RECOVERY"
echo ""

log_info "Waiting 10 seconds for re-delegated step to be processed..."
sleep 10
...
log_info "ValidateCustomer status after recovery: $FINAL_STEP_STATUS"
echo ""

# If re-delegation happened, the step should progress past DELEGATED
# It could be in_progress, completed, or even delegated again (if re-sent)
if [ "$FINAL_STEP_STATUS" = "delegated" ]; then
  # Check if the started_at was updated (re-delegation resets the timer)
  STEP_AGE=$(docker exec ${COMPOSE_PROJECT_NAME:-dtm}-db \
    psql -U dtm_user -d dtm -t -c \
    "SELECT EXTRACT(EPOCH FROM (NOW() - started_at))::int
     FROM dtm_steps WHERE id = '$TARGET_STEP_ID';" | tr -d '[:space:]')

  if [ "$STEP_AGE" -lt 60 ]; then
    log_success "Step was re-delegated (started_at refreshed, age: ${STEP_AGE}s)"
    log_info "Step is still in DELEGATED but was recently re-sent to SQS"
  else
    log_warning "Step is still in old DELEGATED state (age: ${STEP_AGE}s)"
    log_info "Re-delegation may not have refreshed started_at"
  fi
else
  case "$FINAL_STEP_STATUS" in
    in_progress|completed|waiting_for_ack)
      log_success "Step progressed to: $FINAL_STEP_STATUS (re-delegation worked)"
      ;;
    failed)
      log_warning "Step failed after re-delegation: $FINAL_STEP_STATUS"
      log_info "This is acceptable - the re-delegation mechanism worked"
      ;;
    *)
...
log_success "Stuck delegated recovery test completed successfully"
log_info "Summary:"
log_info "  - Job completed in quick-order mode"
log_info "  - ValidateCustomer manually set to DELEGATED (15 min ago)"
log_info "  - Job set back to PROCESSING"
log_info "  - Maintenance task executed: found=$STUCK_FOUND, reDelegated=$RE_DELEGATED"
...
log_info "  - Lost SQS messages"
log_info "  - SQS poller downtime"
log_info "  - Lambda deployment failures"
log_info "  - Queue URL misconfigurations"
echo ""
