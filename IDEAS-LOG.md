...
### plan-execution workflow — dynamic per-job step DAGs (dynamicSteps) — MOSTLY DONE
**Priority:** High (remaining: SEs)
**Effort:** ~2h remaining (SEs only)
**Status (2026-03-31):** Phases 0-4 complete. Workflow registered, an external worker polling SQS for its steps, events relay connected, HTTP ACK endpoint added. Key DTM engine changes:
...
**Priority:** Medium
**Effort:** ~4h
Extract the monitor dashboard components (`JobList`, `JobDetail`, `StepRow`, `EventLog`, `ProgressBar`) into a reusable package that can be embedded in other Preact apps. Currently a standalone Vite app (`apps/monitor/`). The WebSocket hook (`useWebSocket`) is already cleanly separated.
