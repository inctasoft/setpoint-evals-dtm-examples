#!/bin/bash
# ============================================================================
# Order-Processing — Simulator STE Runner
# ============================================================================
#
# Builds the workflow, starts the simulator stack, runs all STEs,
...
#   --no-teardown    Leave the simulator running after STEs complete
...
echo ""
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  Order-Processing — Simulator STE Runner                  ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""
...
# Step 3: Run STEs
# ────────────────────────────────────────────────────────────────
echo "━━━ Step 3: Running order-processing STEs ━━━"
echo ""

# Set environment variables for STE execution against the simulator
...
# Run the workflow's STE suite
STE_DIR="$REPO_ROOT/workflows/order-processing/ste"
STE_EXIT_CODE=0

if [ -f "$STE_DIR/run-all.sh" ]; then
  bash "$STE_DIR/run-all.sh" || STE_EXIT_CODE=$?
else
  echo "  ❌ STE runner not found at $STE_DIR/run-all.sh"
  STE_EXIT_CODE=1
fi
...
# ────────────────────────────────────────────────────────────────
echo "╔════════════════════════════════════════════════════════════════╗"
if [ $STE_EXIT_CODE -eq 0 ]; then
  echo "║  ✅ All STEs PASSED                                          ║"
else
  echo "║  ❌ Some STEs FAILED (exit code: $STE_EXIT_CODE)                          ║"
fi
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""
