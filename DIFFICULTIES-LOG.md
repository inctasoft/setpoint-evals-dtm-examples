...
### Stale .env: postinstall never refreshes an existing .env — new vars silently missing
**Severity:** Medium
**Status:** Fixed (2026-07-17)
postinstall creates .env from .env.example only when MISSING; a checkout whose .env predates DISABLE_AUTH/ENABLE_EVAL_RUN_API boots an auth-walled orchestrator → every SE 401s ("Expected 201, got 404/401" wall, 0% pass). Bit the Phase-4c bring-up on the main checkout. Fixed by adding `check_env_freshness()` (`scripts/inc/common.sh`), called at the top of `local-env.sh`'s `start_standalone`/`start_integrated`: diffs every `KEY=` in `.env.example` against `.env`, prints every missing key by name (loud, not silent), and auto-appends them verbatim from `.env.example` under a dated marker comment. Idempotent (re-running with nothing missing is a silent no-op — verified). Proven live against the actual aged main-checkout `.env`: stripping `DISABLE_AUTH` and re-running surfaced **21** missing keys (not just the 2 originally suspected — this `.env` had drifted further than known), all auto-appended, second run clean. Documented in README Quick Start (a bare `docker restart` does NOT re-read `.env` — recreate with the same `--env-file`/profiles `local-env.sh` uses).

### dist/migrations/*.js survives a schema-consolidation refactor — Postgres 42710 duplicate_object on a fresh bootstrap
**Severity:** High
**Status:** Fixed (2026-07-17)
`packages/database/dist/` is gitignored and `"build": "tsc"` never cleaned it, so a long-lived checkout that predates the Phase 2a "single schema source of truth" consolidation kept 11 compiled migration `.js` files (`1765443716000-InitialMigrationSchema.js` .. `1765443726000-RenameChildEntityIdToChildItemId.js`) in `dist/migrations/` even after their `.ts` sources were squashed into one `1784147958000-InitialSchema.ts`. `packages/database/src/config/typeorm.config.ts`'s migrations glob (`dist/migrations/*.js`) has no way to distinguish current from orphaned — it ran all 12 in timestamp order; the stale ones (smaller epoch) recreated `job_status_enum` etc. first, then the real `InitialSchema` migration hit Postgres `42710` (`DefineEnum`, "type already exists") trying to create it again. This is what broke the Fix-4c bring-up: `scripts/init-clean-database.sh` (SE-14's bootstrap path) dropped the schema and then failed to rebuild it, cascading into SE-15 (0/15 seeded), SE-17/SE-19 (500 "relation dtm_jobs does not exist"), and all three workflow suites ("no jobId returned"). Reproduced live: manual `npx typeorm-ts-node-commonjs migration:run` on the broken checkout ran `RenameChildEntityIdToChildItemId1765443726000` successfully, then failed on `InitialSchema`'s `CREATE TYPE "job_status_enum"`. Fixed: `packages/database/package.json` `"build": "rm -rf dist && tsc"` — a src-side migration consolidation can no longer leave orphaned compiled outputs behind. **SE-14 cannot guard this itself** (both its Path A and Path B read the same stale glob, so they'd be corrupted identically with no differential signal) — added `setpoint-evals/SE-24-database-build-cleans-orphaned-migrations` to guard the build step directly, RED-proofed against the pre-fix bare `tsc` script. Also added an isolation guard to SE-14 itself (`dtm_jobs` restore-on-fail, see its README) so a bootstrap failure — from this cause or any other — can no longer cascade into every SE/workflow-suite that runs after it in the same pass.
