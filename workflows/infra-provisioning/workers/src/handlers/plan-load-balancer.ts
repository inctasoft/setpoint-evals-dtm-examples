...
/**
 * Plan Load Balancer Worker Lambda Handler
 *
 * Plans load balancer configuration from the infra-provisioning source database
 * by load_balancer_id and sends it to the orchestrator via HTTP callback.
 */

const STEP_NAME = "Plan Load Balancer";

async function readLoadBalancerData(
  ...
  const repository = InfraProvisioningDataSource.getRepository(LoadBalancer);

  logger.log(`Querying load_balancers by lb_id:`, loadBalancerId);
  const loadBalancer = await repository.findOne({
    where: { lbId: loadBalancerId },
  });

  if (!loadBalancer) {
    logger.warn(`Load balancer not found:`, loadBalancerId);
    return null;
  }

  logger.log(`Load balancer found:`, {
    lbId: loadBalancer.lbId,
    networkId: loadBalancer.networkId,
    instanceId: loadBalancer.instanceId,
...
async function processPlanWork(
  ...
  const loadBalancerId = String(filterValue);

  logger.log(`Extracted filter value:`, { filterKey: sourceConfig.filterKey, loadBalancerId });
  ...
  try {
    ...
    const loadBalancerData = await readLoadBalancerData(loadBalancerId, logger);

    if (!loadBalancerData) {
      throw new Error(`Load balancer ${loadBalancerId} not found in source database`);
    }

    logger.log(`Sending data to orchestrator...`);
    await sendSuccessCallback(callbackUrl, jobId, stepId, { loadBalancer: loadBalancerData }, 1, retryMetadata, STEP_NAME);
