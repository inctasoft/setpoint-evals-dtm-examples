...
/**
 * Plan Network Worker Lambda Handler
 *
 * This lambda plans network/VPC configuration data from the infra-provisioning
 * source database and sends it to the orchestrator via HTTP callback.
 *
 * Flow:
 * 1. Plan: Read network data using TypeORM by environment_id
 * 2. Callback: Send full network data to orchestrator via HTTP
 */

const STEP_NAME = "Plan Network";

/**
 * Read network data from the infra-provisioning source database
 */
async function readNetworkData(
  ...
  const repository = InfraProvisioningDataSource.getRepository(Network);

  logger.log(`Querying networks by environment_id:`, environmentId);
  const network = await repository.findOne({
    where: { environmentId },
  });

  if (!network) {
    logger.warn(`Network not found for environment:`, environmentId);
    return null;
  }
...
async function processPlanWork(
  ...
  const networkId = String(filterValue);
  ...
  try {
    ...
    const networkData = await readNetworkData(networkId, logger);

    if (!networkData) {
      throw new Error(`Network for ${networkId} not found in source database`);
    }
