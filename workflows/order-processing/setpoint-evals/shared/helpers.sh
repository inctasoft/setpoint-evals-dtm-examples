...
# ═══════════════════════════════════════════════════════════════════════════
# Order Processing Workflow — SE Helpers (Workflow Layer)
# ═══════════════════════════════════════════════════════════════════════════
# Workflow-specific functions for order-processing SEs.
...
#
# This file adds:
#   - Order-processing-specific API functions (initiate_job, get_job_status)
#   - Workflow endpoint configuration (generic workflow API)
#   - Order-processing test data defaults (ENTITY_ID, VARIANT)
...
# Compute repo root from this file's location (4 levels up from workflows/order-processing/setpoint-evals/shared/)
...
# ═══════════════════════════════════════════════════════════════════════════
# Helper: Initiate an order-processing job
# ═══════════════════════════════════════════════════════════════════════════
# POSTs to $ORCHESTRATOR_URL/workflows/order-processing/jobs
# Usage: initiate_job '{"payload": {"customerId": 1000}, ...}'
...
poll_job() {
  ...
  while [ $poll_count -lt $max_polls ]; do
    ...
    local vc_status=$(extract_step_status "$status_response" "ValidateCustomer" || echo "PENDING")
    local vp_status=$(extract_step_status "$status_response" "ValidateProduct" || echo "PENDING")
    local sc_status=$(extract_step_status "$status_response" "SubmitCustomer" || echo "PENDING")
    local vo_status=$(extract_step_status "$status_response" "ValidateOrder" || echo "PENDING")
    local so_status=$(extract_step_status "$status_response" "SubmitOrder" || echo "PENDING")

    echo -e "[${poll_count}/${max_polls}] Job: ${CYAN}${job_status}${NC} | VC=${vc_status} VP=${vp_status} SC=${sc_status} VO=${vo_status} SO=${so_status}"
...
validate_env_for_ste() {
  log_info "Validating environment for order-processing SEs..."
  ...
  # Check order-processing source database container is running
  if ! docker ps --format '{{.Names}}' 2>/dev/null | grep -q "dtm-order-processing-source-db"; then
    log_warning "Order-processing source database container is NOT running"
    log_warning "  Fix: docker compose -f workflows/order-processing/docker-compose.order-processing.yml up -d"
  fi
