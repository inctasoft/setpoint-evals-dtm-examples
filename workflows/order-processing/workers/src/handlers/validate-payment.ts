...
/**
 * Validate Payment Worker Lambda Handler
 *
 * This lambda validates that payment data exists in the order-processing source database
 * by order_id and sends it to the orchestrator via HTTP callback.
 *
 * Flow:
 * 1. Validate: Read payment records using TypeORM by order_id
 * 2. Callback: Send payment data to orchestrator via HTTP
 */

const STEP_NAME = "Validate Payment";

/**
 * Validate and fetch payment data from the order-processing source database
 */
async function extractPaymentData(
  ...
  const repository = OrderProcessingDataSource.getRepository(Payment);

  logger.log(`Querying payments for order_id:`, orderId);
  const payments = await repository.find({
    where: { orderId },
  });

  if (payments.length === 0) {
    logger.warn(`No payments found for order_id:`, orderId);
    return [];
  }
...
async function processSourceWork(
  ...
  try {
    logger.log(`Validating payment in order-processing database...`);
    const payments = await extractPaymentData(orderId, logger);

    if (payments.length === 0) {
      throw new Error(`No payments found for order ${orderId} in source database`);
    }
