...
/**
 * Validate Order Worker Lambda Handler
 *
 * This lambda validates that an order exists in the order-processing source database
 * and sends the order details to the orchestrator via HTTP callback.
 *
 * Flow:
 * 1. Validate: Read order data using TypeORM by order_id
 * 2. Callback: Send full order data to orchestrator via HTTP
 */

const STEP_NAME = "Validate Order";

/**
 * Validate and fetch order data from the order-processing source database
 */
async function extractOrderData(
  ...
  const repository = OrderProcessingDataSource.getRepository(Order);

  logger.log(`Querying order_id:`, orderId);
...
async function processSourceWork(
  ...
  try {
    logger.log(`Validating order in order-processing database...`);
    const orderData = await extractOrderData(orderId, logger);

    if (!orderData) {
      throw new Error(`Order ${orderId} not found in source database`);
    }
