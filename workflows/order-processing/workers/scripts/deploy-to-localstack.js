 ...
 *
 * Order-Processing workflow handlers:
 * - Customer: validate + submit (root entity, no fan-out)
 * - Product: validate only (root entity, no submit)
 * - Order: validate + submit (child entity, depends on customer)
 * - LineItem: discover + validate + submit (fan-out pattern)
 * - Payment: validate + submit (child entity, depends on order)
 * - Shipment: validate + submit (child entity, depends on order)
 */
const HANDLERS = {
  ...
  "order-submit-customer": {
    ...
    queue: "order-submit-customer",
    needsDb: false,
  },

  // Product (root entity, validate-only)
  "order-validate-product": {
    handler: "validate-product",
    queue: "order-validate-product",
    needsDb: true,
  },
  ...
  "order-validate-order": {
    ...
    queue: "order-validate-order",
    needsDb: true,
  },
  "order-submit-order": {
    handler: "submit-order",
    queue: "order-submit-order",
    needsDb: false,
  },

  // Line Items - Fan-out pattern (child of order)
  "order-discover-line-items": {
    handler: "discover-line-items",
    queue: "order-discover-line-items",
    needsDb: true,
  },
  "order-validate-line-item": {
    handler: "validate-line-item",
    queue: "order-validate-line-item",
    needsDb: true,
  },
  "order-submit-line-item": {
    handler: "submit-line-item",
    queue: "order-submit-line-item",
    needsDb: false,
  },
  ...
  "order-validate-payment": {
    ...
    queue: "order-validate-payment",
    needsDb: true,
  },
  "order-submit-payment": {
    handler: "submit-payment",
    queue: "order-submit-payment",
    needsDb: false,
  },

  // Shipment (child of order)
  "order-validate-shipment": {
    handler: "validate-shipment",
    queue: "order-validate-shipment",
    needsDb: true,
  },
  "order-submit-shipment": {
    handler: "submit-shipment",
    queue: "order-submit-shipment",
    needsDb: false,
  },

  // Archive (final step — writes to product DB)
  "order-archive-processed-order": {
    handler: "archive-processed-order",
    queue: "order-archive-processed-order",
    needsDb: false,
    needsProductDb: true,
  },
};
...
function deployFunction(functionName, config, isHotReload) {
  ...
  // Add DB connection for validate/discover handlers (they query source DB)
  if (config.needsDb) {
    Object.assign(envVars, {
      ORDER_PROCESSING_DB_HOST: "dtm-db",
  ...
  if (config.needsProductDb) {
    Object.assign(envVars, {
      ORDER_PROCESSING_PRODUCT_DB_HOST: "dtm-db",
      ORDER_PROCESSING_PRODUCT_DB_PORT: "5432",
      ORDER_PROCESSING_PRODUCT_DB_USER: "order_user",
      ORDER_PROCESSING_PRODUCT_DB_PASSWORD: "order_pass",
      ORDER_PROCESSING_PRODUCT_DB_NAME: "order_processing_product_db",
    });
  }
  ...
  try {
    ...
    if (isHotReload) {
      ...
      const handlerPath = `workflows/order-processing/workers/dist/${config.handler}/index.handler`;
...
function main() {
  log("╔════════════════════════════════════════════════╗", colors.blue);
  log("║   Order-Processing Lambda Workers Deployment  ║", colors.blue);
  log("╚════════════════════════════════════════════════╝", colors.blue);
  console.log();
