...
# ═══════════════════════════════════════════════════════════════════════════
# IoT Sensor Pipeline Workflow — SE Helpers (Workflow Layer)
# ═══════════════════════════════════════════════════════════════════════════
# Workflow-specific functions for iot-sensor-pipeline SEs.
# Chains to the generic SE helpers for core functionality.
#
# Usage from workflow SEs:
#   source "${SCRIPT_DIR}/../shared/helpers.sh"
#
# This file adds:
#   - IoT-sensor-pipeline-specific API functions (initiate_job, get_job_status)
#   - Workflow endpoint configuration (generic workflow API)
#   - IoT-sensor-pipeline test data defaults (ENTITY_ID, VARIANT)
...
# Compute repo root from this file's location (4 levels up from workflows/iot-sensor-pipeline/setpoint-evals/shared/)
WORKFLOW_SE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
...
source "${_WORKFLOW_REPO_ROOT}/setpoint-evals/shared/helpers.sh"
...
# ═══════════════════════════════════════════════════════════════════════════
# Helper: Initiate an iot-sensor-pipeline job
# ═══════════════════════════════════════════════════════════════════════════
# POSTs to $ORCHESTRATOR_URL/workflows/iot-sensor-pipeline/jobs
# Usage: initiate_job '{"payload": {"deviceId": "device-001"}, ...}'
...
poll_job() {
  ...
  while [ $poll_count -lt $max_polls ]; do
    ...
    local rd_status=$(extract_step_status "$status_response" "RegisterDevice" || echo "PENDING")
    local pd_status=$(extract_step_status "$status_response" "ProvisionDevice" || echo "PENDING")
    local ds_status=$(extract_step_status "$status_response" "DiscoverSensors" || echo "PENDING")
    local cs_status=$(extract_step_status "$status_response" "CalibrateSensor" || echo "PENDING")
    local as_status=$(extract_step_status "$status_response" "ActivateSensor" || echo "PENDING")

    echo -e "[${poll_count}/${max_polls}] Job: ${CYAN}${job_status}${NC} | RD=${rd_status} PD=${pd_status} DS=${ds_status} CS=${cs_status} AS=${as_status}"
...
validate_env_for_ste() {
  log_info "Validating environment for iot-sensor-pipeline SEs..."
  ...
  # Check iot-sensor-pipeline source database container is running
  if ! docker ps --format '{{.Names}}' 2>/dev/null | grep -q "dtm-iot-sensor-pipeline-source-db"; then
    log_warning "IoT Sensor Pipeline source database container is NOT running"
    log_warning "  Fix: docker compose -f workflows/iot-sensor-pipeline/docker-compose.iot-sensor-pipeline.yml up -d"
  fi
