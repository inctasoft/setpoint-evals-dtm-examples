...
describe('WorkflowJobService', () => {
  ...
  describe('initiateWorkflowJob', () => {
    ...
    it('should create a job successfully with default variant', async () => {
      ...
      // Act
      const result = await service.initiateWorkflowJob('order-processing', baseDto);
    ...
    it('should create a job with explicit variant', async () => {
      ...
      // Act
      const result = await service.initiateWorkflowJob('order-processing', dtoWithVariant);
    ...
    it('should throw NotFoundException for unknown workflow', async () => {
      ...
      // Act & Assert
      await expect(service.initiateWorkflowJob('unknown-workflow', baseDto)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw ForbiddenException for disabled workflow', async () => {
      ...
      // Act & Assert
      await expect(service.initiateWorkflowJob('order-processing', baseDto)).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('should throw BadRequestException for invalid variant', async () => {
      ...
      // Act & Assert
      await expect(
        service.initiateWorkflowJob('order-processing', dtoWithBadVariant),
    ...
    it('should throw ConflictException for duplicate job', async () => {
      ...
      // The service only runs the dedup check when there's a deduplicationKey or payload
      // ("5. Check for duplicate job" block guards on `deduplicationKey`).
      ...
      // Act & Assert
      await expect(service.initiateWorkflowJob('order-processing', dtoWithPayload)).rejects.toThrow(
        ConflictException,
      );
    });
    ...
    it('should pass payload and testOptions through to job creation', async () => {
      ...
      // Act
      await service.initiateWorkflowJob('order-processing', dtoWithPayload);
    ...
    it('should include deduplication override when provided', async () => {
      ...
      // Act
      await service.initiateWorkflowJob('order-processing', dtoWithDedup);
