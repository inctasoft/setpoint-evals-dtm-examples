/**
 * Custom SQS Poller — Simulator Mode
 *
 * Identical to poller.ts but imports from the .simulator variants of
 * queue-discovery and handler-registry, which load workflows dynamically
 * from environment variables instead of hardcoded imports.
 *
 * Additionally auto-creates SQS queues on startup (no localstack-init.sh needed).
 *
 * DEBUG-SERVER MODE:
...
const SQS_ENDPOINT = process.env.AWS_SQS_ENDPOINT || "http://localstack:4566";
const LAMBDA_ENDPOINT =
  process.env.AWS_LAMBDA_ENDPOINT || "http://localstack:4566";
...
// ─── Simulator: Dynamic queue discovery ─────────────────────────────────────
import { discoverQueuesFromWorkflows } from "./queue-discovery.simulator";
...
// ─── Simulator: Auto-create SQS queues ──────────────────────────────────────

/**
 * Auto-create all discovered queues in LocalStack.
 * In simulator mode there's no localstack-init.sh, so the poller creates
 * queues on startup. CreateQueue is idempotent — safe to call if already exists.
 */
async function ensureQueuesExist(): Promise<void> {
  console.log(`[Simulator] Auto-creating ${QUEUES.length} SQS queue(s)...`);
  ...
  for (const queueName of QUEUES) {
    try {
      ...
      const dlqArn = `arn:aws:sqs:${AWS_REGION}:${AWS_ACCOUNT_ID}:${dlqName}`;
      await sqsClient.send(
      ...
      console.log(`  ✅ ${queueName} (+ DLQ)`);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.warn(`  ⚠️  ${queueName}: ${message}`);
    }
  }
}
...
async function initDebugServerMode(): Promise<void> {
  ...
  console.log("╔════════════════════════════════════════════════╗");
  console.log("║   🔬 SIMULATOR DEBUG-SERVER MODE               ║");
  ...
  try {
    // ─── Simulator: Dynamic import of handler registry ──────────────────
    const registry = await import("./handler-registry.simulator");
    ...
    console.error("   Make sure workflow worker packages are built");
    process.exit(1);
  }
}
  ...
  try {
    ...
    if (messages.length > 0) {
      ...
      try {
        ...
        // Force-delete messages that have exceeded max retries in debug-server mode
...
function printStats(): void {
  ...
  console.log("\n" + "=".repeat(80));
  console.log(
    `📊 SIMULATOR POLLER STATISTICS ${DEBUG_SERVER_MODE ? "(DEBUG-SERVER MODE)" : ""}`,
  );
...
async function main(): Promise<void> {
  // ─── Simulator: Auto-create queues before anything else ─────────────
  ...
  if (!DEBUG_SERVER_MODE) {
    console.log("╔════════════════════════════════════════════════╗");
    console.log("║   SQS Poller — Simulator Mode                 ║");
    console.log("╚════════════════════════════════════════════════╝");
  }
