...
#
# Usage:
#   ./ste/preflight-check.sh [--verbose] [--fix]
#
# Exit Codes:
...
Usage:
  ./ste/preflight-check.sh [OPTIONS]
...
check_database_connectivity() {
  ...
  if docker exec ${COMPOSE_PROJECT_NAME:-dtm}-order-processing-source-db pg_isready -U order_user -d order_processing_db > /dev/null 2>&1; then
    ...
    log_info "Order Processing source DB not running (optional — only needed for order-processing workflow STEs)"
  fi
}
...
check_seed_data() {
  ...
  if docker exec ${COMPOSE_PROJECT_NAME:-dtm}-order-processing-source-db pg_isready -U order_user -d order_processing_db > /dev/null 2>&1; then
    ...
    if [ "$customer_count" -gt "0" ] 2>/dev/null; then
      ...
      log_warning "Order Processing source DB appears empty (workflow STEs may fail)"
    fi
  else
...
main() {
  ...
  log_header "STE PRE-FLIGHT CHECK"
  ...
  if [ "$ERRORS" -eq 0 ] && [ "$WARNINGS" -eq 0 ]; then
    echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${GREEN}✅ ALL CHECKS PASSED - READY TO RUN STEs${NC}"
    echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo "Next steps:"
    echo "  • Run single eval:  cd ste/01-retry-transient-failure && ./test.sh"
    echo "  • Run all evals:    ./ste/run-all.sh"
    echo ""
    exit 0
