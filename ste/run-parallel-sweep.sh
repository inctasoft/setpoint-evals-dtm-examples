...
# for the current environment. Runs the full STE suite at each level and
# reports a comparison table.
#
# Usage:
#   ./ste/run-parallel-sweep.sh [OPTIONS]
#
# Options:
#   --values "4 6 8 10 0"   Space-separated max-parallel values to test (0=unlimited)
#   --runs-per-value N       Runs per value (default: 2)
#   --all-workflows          Include workflow STEs (default: core only)
#   --help                   Show this help
#
# Example:
#   ./ste/run-parallel-sweep.sh --values "4 6 8 12" --runs-per-value 3 --all-workflows
#
################################################################################
...
echo ""
echo "To apply:"
echo "  ./ste/run-all.sh --max-parallel=$BEST_VALUE --all-workflows"
echo ""
