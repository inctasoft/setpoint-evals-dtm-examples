...
# ═══════════════════════════════════════════════════════════════════════════
# DTM Core — STE Shared Helpers (Generic Layer)
# ═══════════════════════════════════════════════════════════════════════════
# Generic functions for State Transition Evals (STEs).
# These work with any workflow loaded by the DTM engine.
#
# Usage from core STEs:
#   source "${SCRIPT_DIR}/../shared/helpers.sh"
#
# Usage from workflow STEs (via chaining):
...
# Determine the repository root (2 levels up from ste/shared/)
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"

# Also set EVAL_ROOT_DIR for backward compatibility
EVAL_ROOT_DIR="$REPO_ROOT"

# Load from .env.local for STEs (they run from host, not Docker)
...
validate_env_for_ste() {
  log_info "Validating environment for STE execution..."
